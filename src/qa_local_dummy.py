...
# ---------- Function to detect PDF changes ----------
def get_pdf_hashes():
    """Generate a hash based on file names and modification times."""
...
def needs_rebuild():
    """Check if FAISS index or metadata file is missing or outdated."""
...
def watch_for_pdf_changes(interval=15):
    """Background thread that checks for PDF changes periodically."""
    ...
    while True:
        ...
        try:
            ...
            if current_hash != last_hash:
                ...
                # Reload FAISS index and metadata dynamically
                global index, metadata
                index = faiss.read_index(INDEX_FILE)
                with open(METADATA_FILE, "r", encoding="utf-8") as f:
                    metadata = [line.strip() for line in f.readlines()]

                print("✅ Index rebuilt and reloaded successfully.\n")
...
Do NOT add new information outside the staff notes.
        """
    else:
        prompt = f"""
...
Answer:
    ...
    print("\n📘 Final Answer (based on staff notes):\n")
    print(final_answer)
    print("\n" + "-" * 80 + "\n")
    return final_answer
...
if __name__ == "__main__":
    ...
    print("Ask a question based on staff notes or say commands like:\n"
          "'shorten to 2 marks', 'expand to 13 marks', 'simplify', 'add example', etc.")
